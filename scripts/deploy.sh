...
deploy_ios() {
  ...
  # 이전 빌드 결과물 삭제 (오래된 파일 재업로드 방지)
  rm -rf "$PROJECT_DIR/build/ios/archive"
  rm -rf "$PROJECT_DIR/build/ios/ipa"

  # 1단계: Archive + IPA 생성 (development 서명 — 자동 서명 사용)
  if ! flutter build ios --no-codesign 2>&1; then
    print_error "iOS 빌드에 실패했습니다."
    return 1
  fi

  # 2단계: Archive 생성
  print_step "iOS Archive 생성 중..."
  if ! flutter build ipa --export-method development 2>&1; then
    # flutter build ipa가 IPA export에서 타임아웃될 수 있음 → xcodebuild로 재시도
    ARCHIVE_PATH="$PROJECT_DIR/build/ios/archive/Runner.xcarchive"
    if [ ! -d "$ARCHIVE_PATH" ]; then
      print_error "Archive 생성에 실패했습니다."
      return 1
    fi

    print_warning "IPA export 타임아웃, xcodebuild로 재시도..."
    rm -rf "$PROJECT_DIR/build/ios/ipa"

    if ! xcodebuild -exportArchive \
      -archivePath "$ARCHIVE_PATH" \
      -exportPath "$PROJECT_DIR/build/ios/ipa" \
      -exportOptionsPlist "$EXPORT_OPTIONS" 2>&1; then
      print_error "IPA export에 실패했습니다."
      return 1
    fi
  fi
  ...
  if [ -z "$IPA_PATH" ]; then
    print_error "IPA 파일이 생성되지 않았습니다."
    return 1
  fi
...
deploy_android() {
  ...
  # 이전 빌드 결과물 삭제 (오래된 파일 재업로드 방지)
  rm -f "$PROJECT_DIR/build/app/outputs/flutter-apk/app-release.apk"

  # APK 빌드
  if ! flutter build apk --release 2>&1; then
    print_error "Android APK 빌드에 실패했습니다."
    return 1
  fi

  APK_PATH="$PROJECT_DIR/build/app/outputs/flutter-apk/app-release.apk"
  if [ ! -f "$APK_PATH" ]; then
    print_error "APK 파일이 생성되지 않았습니다."
    return 1
  fi
...
case "$PLATFORM" in
  ios)
    deploy_ios || HAS_ERROR=true
    ;;
  android)
    deploy_android || HAS_ERROR=true
    ;;
  all)
    deploy_ios || HAS_ERROR=true
    deploy_android || HAS_ERROR=true
...
if [ "$HAS_ERROR" = true ]; then
  echo -e "${RED}================================================${NC}"
  echo -e "${RED}  일부 배포 실패! (${MINUTES}분 ${SECONDS}초)${NC}"
