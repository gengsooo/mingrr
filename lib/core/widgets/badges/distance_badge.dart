...
enum DistanceBadgeSize {
  ...
  const DistanceBadgeSize(this.iconSize);
  
  /// 크기별 TextStyle 반환
  TextStyle getTextStyle(BuildContext context) {
    switch (this) {
      case DistanceBadgeSize.small:
        return AppTextStyles.labelMedium(context);  // 11px
      case DistanceBadgeSize.medium:
        return AppTextStyles.bodySmall(context);    // 12px
      case DistanceBadgeSize.large:
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
      ...
      children: [
        ...
        Text(
          distanceString,
          style: size.getTextStyle(context).withColor(effectiveColor),
        ),
      ],
    );
  }
