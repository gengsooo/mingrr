...
class _WalkScreenState extends ConsumerState<WalkScreen> {
  ...
  Future<void> _showLocationErrorDialog() async {
    ...
    final dialogResult = await showErrorDialog(
      ...
      themeColor: context.features.health,
    );
  ...
  Future<void> _drawRoutePolyline() async {
    ...
    try {
      ...
      await _mapController!.routeLayer.addRoute(
        points,
        RouteStyle(context.features.health, 8),
      );
        ...
        child: Column(
          ...
          children: [
            ...
            MingrrButton(
              ...
              backgroundColor: _isWalking ? Theme.of(context).colorScheme.error : context.features.health,
                  ...
                  if (snapshot.hasError) {
                    ...
                    return Center(
                      child: Column(
                        mainAxisSize: MainAxisSize.min,
                        children: [
                          Icon(AppIcons.error, size: 64, color: Theme.of(context).colorScheme.error),
                          const SizedBox(height: AppSizes.gapL),
                          Text(
                            '산책 기록을 불러올 수 없어요',
